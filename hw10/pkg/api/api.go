...
var (
	...
	indexWordsTotal = promauto.NewGauge(prometheus.GaugeOpts{
		Name: "index_words_total",
		Help: "Количество индексированных слов.",
	})
	storageDocumentsTotal = promauto.NewGauge(prometheus.GaugeOpts{
		Name: "storage_documents_total",
		Help: "Количество документов в хранилище.",
	})
	searchRequestsLength = promauto.NewCounter(prometheus.CounterOpts{
		Name: "search_request_length",
		Help: "Сумма длин поисковых запросов, байт.",
	})
)
...
func (s *Service) Collector() {
	...
	for _ = range ticker.C {
		storageDocumentsTotal.Set(float64(s.engine.LenStorage()))
		indexWordsTotal.Set(float64(s.engine.LenIndex()))
	}
}
